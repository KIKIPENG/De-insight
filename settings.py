...
class SettingsScreen(ModalScreen[str | None]):
    ...
    def _save_chat_model(self, pid, model_name, full_id, pinfo):
        ...
        if pinfo.get("key_env"):
            ...
            if key_val and pinfo["key_env"] != "ANTHROPIC_API_KEY":
                self._env["OPENAI_API_KEY"] = key_val
        if pinfo.get("default_base"):
            base_val = self._env.get(pinfo.get("base_env", ""), "") or pinfo["default_base"]
            self._env["OPENAI_API_BASE"] = base_val
        elif not pinfo.get("base_env"):
    ...
    def _save_rag_model(self, pid, model_name, full_id, pinfo):
        self._env["RAG_LLM_MODEL"] = model_name
        if pinfo.get("key_env"):
